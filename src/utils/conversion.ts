...
export function buildInventoryId(size: ParsedSize, supplier: Supplier, finish?: string): string {
  ...
  console.log(`[BUILD_INV_ID] Input: thickness=${size.thickness}, width=${size.width}, length=${size.length}, supplier=${supplier}, finish=${finish}`);
  ...
  console.log(`[BUILD_INV_ID] Manual mapping result:`, mapping);
  if (mapping) {
    console.log(`[BUILD_INV_ID] USING MANUAL MAPPING: ${mapping.inventoryId}`);
  ...
  console.log(`[BUILD_INV_ID] Uploaded inventory match: ${uploadedMatch}`);
  if (uploadedMatch) {
    console.log(`[BUILD_INV_ID] USING UPLOADED INVENTORY: ${uploadedMatch}`);
    return uploadedMatch;
  }

  // 3. Auto-generate with proper Acumatica padding
  const autoGenerated = formatInventoryId(size.thickness, size.width, size.length, '304/304L', finishCode);
  console.log(`[BUILD_INV_ID] AUTO-GENERATED: ${autoGenerated}`);
